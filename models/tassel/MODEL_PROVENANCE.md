...
| Runtime filename | `tassel-best.pt` |
| Runtime path | `models/deployment/tassel-best.pt` |
...
name identifies the task explicitly, and `backend/inference.py` loads it from
the configurable deployment-model path.

Acceptance inference on `examples/tassel-counting/output/DJI_0243 (2)_annotated.jpg` produced 121
